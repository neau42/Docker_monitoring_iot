...
HASH_PASSWORD=$(echo "$MAIN_ADMIN_TOKEN" | node-red-admin hash-pw | head -1 | awk '{print $2}') \
  && echo HASH_PASSWORD=$HASH_PASSWORD \
  && sed -i "s|__HASH_ADMIN_PASSWORD__|$HASH_PASSWORD|g" /data/settings.js

# Set user password
HASH_USER_PASSWORD=$(echo "$MAIN_USER_PASSWORD" | node-red-admin hash-pw | head -1 | awk '{print $2}') \
